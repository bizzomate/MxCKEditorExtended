define([
    ...
    return declare("CKEditorForMendix.widget.CKEditorForMendix", [_WidgetBase, _TemplatedMixin], {
        ...
        _setAutoSaveFeature : function(){
            if(this.autoSaveEnabled){
                if (this.notificationEnabled){
                    // add general class for customization
                    document.body.classList.add('cke_autosave_notification');
                    this._setNotificationPosition();
                }
                setInterval(lang.hitch(this,function(){
                    // assign editor content to the data source attr 
                this._editorChange(this._editor.getData());
                // commit content changes - save action
                if (this._contextObj !== null) {
                    mx.data.commit({
                        ...
                        callback: lang.hitch(this,function() {
                            ...
                            console.debug("content auto saved successfully!");
                        }),
                        error: lang.hitch(this,function(e) {
                            if (this.notificationEnabled){
                                this._editor.showNotification( 'Error while trying to save content', 'error' );
                            }
                            console.error("Error while trying to save content",e);
                        })
                    });
                }
        ...
        _setNotificationPosition : function(){
            document.body.classList.add(`cke_autosave_notification_pos_${this.notificationPosition}`);
        },
